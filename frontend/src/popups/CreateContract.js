    ...
    return (
        <>
            <Listbox value={selectedEndpoint} onChange={setSelectedEndpoint}>
                <Listbox.Button className="relative flex items-center w-full cursor-pointer rounded-lg py-2 pl-3 pr-10 text-left border border-gray-300 text-gray-700">
                ...
                <div className="relative w-full">
                    <Listbox.Options className="absolute width-full mt-1 max-h-60 w-full rounded-md bg-white py-1 text-base border border-gray-300">
                        {endpoints.map((ep) => (
                            <Listbox.Option
                                key={ep["id"]}
                                value={ep}
                                className="relative items-center flex space-x-2 cursor-default select-none py-2 pl-4 pr-4 text-gray-700">
